// The states a route search lands in when there's no card to show. Every one of them says what
...
const EMPTY_COPY: Record<RouteEmptyReason, EmptyCopy> = {
  "pick-places": {
    ...
    tone: "bg-zinc-500/10 text-zinc-600 ring-zinc-500/20 dark:text-zinc-300 dark:ring-white/10",
  },
  "same-place": {
    Icon: Route,
    title: "Pareho ang simula at destinasyon",
    body: "Nandiyan ka na! Pumili ng ibang destinasyon para makabuo ng ruta.",
    tone: "bg-sky-500/12 text-sky-600 ring-sky-500/25 dark:text-sky-400 dark:ring-sky-400/25",
  },
  "no-route": {
    Icon: TriangleAlert,
    ...
    tone: "bg-amber-500/12 text-amber-600 ring-amber-500/25 dark:text-amber-400 dark:ring-amber-400/25",
  },
}
...
export function RouteEmptyState({ reason, actionLabel, onAction, className }: RouteEmptyStateProps) {
  ...
  return (
    <motion.div
      ...
      transition={spring}
      className={cn(
        "w-full max-w-md rounded-3xl px-5 py-8 text-center",
    ...
    >
      ...
      <p className="mx-auto mt-1.5 max-w-xs text-sm leading-relaxed text-zinc-500 dark:text-zinc-400">{body}</p>
      ...
      {actionLabel !== undefined && onAction !== undefined && (
        <button
          ...
          onClick={onAction}
          className={cn(
            "mt-4 inline-flex min-h-11 items-center justify-center rounded-2xl px-5 text-sm font-semibold",
            ...
            "dark:bg-zinc-50 dark:text-zinc-900 dark:hover:bg-zinc-200 dark:focus-visible:ring-white/30"
          )}
        >
          {actionLabel}
        </button>
