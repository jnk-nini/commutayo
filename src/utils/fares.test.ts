...
describe("discountedFare", () => {
  ...
  it("takes 20% off for students, seniors, and PWDs, who all share one class", () => {
    expect(discountedFare(25, "discounted")).toBe(20)
    expect(FARE_CLASSES.filter((option) => option.discount > 0)).toHaveLength(1)
  })
  ...
  it("rounds the discount to cash a conductor can hand back", () => {
    ...
    expect(discountedFare(13, "discounted")).toBe(10)
    expect(discountedFare(19, "discounted")).toBe(15)
    expect(discountedFare(18, "discounted")).toBe(14)
  })
...
describe("discountedTotalFare", () => {
  ...
  it("saves money for a discounted rider without going negative", () => {
    const discounted = discountedTotalFare(route, "discounted")
    expect(discounted).toBeLessThan(route.totalFare)
    expect(discounted).toBeGreaterThan(0)
    expect(fareSavings(route, "discounted")).toBeCloseTo(route.totalFare - discounted)
  })

  it("discounts each leg the way a conductor would, not the grand total", () => {
    const perLeg = route.steps.reduce((sum, step) => sum + discountedFare(step.fare, "discounted"), 0)
    expect(discountedTotalFare(route, "discounted")).toBeCloseTo(perLeg)
  })
})
